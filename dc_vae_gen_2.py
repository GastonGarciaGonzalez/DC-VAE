...
class DCVAE:
    ...
    def __init__(self,
        ...
        self.__name = name
        self.__save_best_model = save_best_model
        self.__save_complete_model = save_complete_model
    ...
    def fit(self, df_X=None, val_percent=0.1, seed=42):
        ...
        cb.append(keras.callbacks.EarlyStopping(min_delta=1e-2,
                                                      ...
                                                      verbose=1,
                                                      mode='min'))
        if self.__save_best_model:
            cb.append(keras.callbacks.ModelCheckpoint(
                filepath=self.__name+'_best_model.h5',
                verbose=1,
                mode='min',
                save_best_only=True))
        ...
        # Save models
        if self.__save_complete_model:
            self.encoder.save(self.__name+'_encoder.h5')
            self.decoder.save(self.__name+'_decoder.h5')
            self.vae.save(self.__name+'_complete.h5')
    ...
    def predict(self,
        ...
        # Trained model
        if load_model:
            self.vae = keras.models.load_model(self.__name+'_complete.h5',
                                                    custom_objects={'sampling': Sampling},
                                                    compile = True)
            self.encoder = keras.models.load_model(self.__name+'_encoder.h5',
                                                    custom_objects={'sampling': Sampling},
                                                    compile = True)
        ...
        if len(alpha_set_up) == self.__M:
            ...
            with open(self.__name + '_alpha_up.pkl', 'rb') as f:
        ...
        if len(alpha_set_down) == self.__M:
            ...
            with open(self.__name + '_alpha_down.pkl', 'rb') as f:
