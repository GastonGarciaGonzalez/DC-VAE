...
class DCVAE:
    ...
    def __init__(self,
        ...
        # =============================================================================
        # Input
        latent_inputs = Input(shape=(sequence_length, latent_dim), name='z_sampling')
        ...
        # Lastest/Output
        x__mean = Conv1D(number_of_vars, kernel, activation=None, 
                                  padding="causal",
                                  name='x__mean_output')(h_dec_cnn)
        x_log_var = Conv1D(number_of_vars, kernel, activation=None,
                                  padding="causal", 
    ...
    def fit(self, df_X=None, val_percent=0.1, seed=42):
        ...
        dataset_train = timeseries_dataset_from_array(
            X, None, self.sequence_length, sequence_stride=1, sampling_rate=1,
    ...
    def point_operation(self, load_model=False, df_X=None, df_y=None, scaler=None, custom_metrics=False, alpha=0, 
        ...
        X_evaluate = X[self.sequence_length-1:]
        y_evaluate = y[self.sequence_length-1:]
        

        
        print('Alpha selection...')
        best_f1 = np.zeros(self.number_of_vars)
        max_alpha = 7
        best_alpha_up = max_alpha*np.ones(self.number_of_vars)
        best_alpha_down = max_alpha*np.ones(self.number_of_vars)
        for alpha_up in np.arange(max_alpha, 1, -1):
            for alpha_down in np.arange(max_alpha, 1, -1):
                ...
                for c in range(self.number_of_vars):
                    if custom_metrics:
    ...
    def predict(self, load_model=False,
        ...
        df_sig = pd.DataFrame(sig, columns=df_X.columns, index=df_X.iloc[self.sequence_length-1:].index)
        error = abs(X_evaluate-reconstruct)
        df_score = pd.DataFrame(error, columns=df_X.columns, index=df_X.iloc[self.sequence_length-1:].index)
        
        # Thresholds
        if len(alpha_set_up) == self.number_of_vars:
            alpha_up = np.array(alpha_set_up)
        else:
            alpha_up = self.alpha_up
            
        if len(alpha_set_down) == self.number_of_vars:
        ...
        df_predict = pd.DataFrame(pred, columns=df_X.columns, index=df_X.iloc[self.sequence_length-1:].index)
        
        if only_predict:
            return df_predict
        else:
            df_reconstruct = pd.DataFrame(reconstruct, columns=df_X.columns, index=df_X.iloc[self.sequence_length-1:].index)
            latent_space = self.encoder.predict(data)[2]
            latent_space = latent_space[:,-1,:]
            df_latent_space = pd.DataFrame(latent_space, columns=np.arange(latent_space.shape[-1]), index=df_X.iloc[self.sequence_length-1:].index)
